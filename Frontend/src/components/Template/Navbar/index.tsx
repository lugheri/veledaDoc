...
export const Navbar: React.FC<{side:'open'|'closed'}> = (props) => {
  type ICredential = {
    id: number;
    level_id: number;
    name: string;
  ...
  const getInfoCredential = async () => {
    try{
      const infoC = await api.get(`getCredential/${authenticate?.userData?.credential}`)
      ...
      <div className="flex justify-center items-center">
        ...
        <div className="group text-xl p-2 cursor-pointer flex justify-center items-center">
          ...
          <div className="flex flex-col justify-center items-start">
            <p className="text-sm font-medium text-slate-500">{authenticate?.userData?.name}</p>
